...
class UserViewTestCase(TestCase):
    ...
    def test_login_correct_redirect(self):
        ...
        with self.client as c:
            ...
            self.assertEqual(session[CURR_USER_KEY], self.testuser.id)
            self.assertEqual(g.user, testuser)
    ...
    def test_users_show(self):
        ...
        with self.client as c:
            with c.session_transaction() as session:
                session[CURR_USER_KEY] = self.testuser.id
            testuser2 = db.session.query(User).filter(User.username == "testuser2").first()
    ...
    def test_users_likes(self):
        ...
        with self.client as c:
            with c.session_transaction() as session:
                session[CURR_USER_KEY] = self.testuser.id
            testuser2 = db.session.query(User).filter(User.username=="testuser2").first()
            resp = c.get(f'users/{testuser2.id}/likes')
            html = resp.get_data(as_text=True)
    ...
    def test_add_follow(self):
        ...
        with self.client as c:
            ...
            self.assertEqual(resp.status_code, 302)
            self.assertEqual(resp.location, f'http://localhost/users/{self.testuser2.id}/following')
            self.assertIn(testuser, testuser2.following)
    ...
    def test_add_follow_redirect(self):
        ...
        with self.client as c:
            ...
            testuser = db.session.query(User).filter(User.username == "testuser").first()
            testuser2 = db.session.query(User).filter(User.username =="testuser2").first()
            resp = c.post(f'/users/follow/{testuser.id}', follow_redirects=True)
            html = resp.get_data(as_text=True)
    ...
    def test_stop_following(self):
        ...
        with self.client as c:
            ...
            self.assertNotIn(testuser, testuser2.following)
    ...
    def test_stop_following_redirect(self): 
        ...
        with self.client as c:
            ...
            testuser2 = db.session.query(User).filter(User.username == "testuser2").first()
            testuser = db.session.query(User).filter(User.username == "testuser").first()
            resp = c.post(f'/users/stop-following/{testuser2.id}', follow_redirects=True)
            html = resp.get_data(as_text=True)
    ...
    def test_profile_success(self):
        ...
        with self.client as c:
            ...
            self.assertEqual(testuser.location, 'New York City')
    ...
    def test_add_remove_likes_add(self):
        ...
        with self.client as c:
            with c.session_transaction() as session:
                session[CURR_USER_KEY] = self.testuser.id
            msg2 = db.session.query(Message).filter(Message.text == "testing 2").first()
            resp = c.post(f'/users/add_like/{msg2.id}')
            data = resp.json

            self.assertEqual(resp.status_code, 200)
            self.assertIn('like added', data)
    ...
    def test_add_remove_likes_remove(self):
        ...
        with self.client as c:
            with c.session_transaction() as session:
                session[CURR_USER_KEY] = self.testuser.id
            testuser = db.session.query(User).filter(User.username == "testuser").first()
            testuser.likes.append(self.msg2)
            ...
            self.assertEqual(resp.status_code, 200)
            self.assertIn('like removed', data)
